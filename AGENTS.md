...
- **Cron installer (`cron.sh`)** — `./cron.sh install` writes four crontab entries that wrap `py-stremio-cron` (scan-light every 3h, scan-full every 6h, download every 2h, combined every 12h), all capped at `INTERNET_SPEED_LIMIT=50`. `./cron.sh uninstall` removes only the py-stremio lines, leaving the rest of the user's crontab intact. The script auto-detects the venv binary, respects `PY_STREMIO_ROOT` / `PY_STREMIO_CRON_SPEED` env vars, and only mutates lines matching the `# py-stremio-managed:` marker.
